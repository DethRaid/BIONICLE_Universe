...
#region Styles
	...
	public Pathfinding.Node firstShiftNode;
	...
	public void DrawUserConnections () {
		...
		if (editLinks) {
			...
			if (Event.current.shift) {
				...
				Pathfinding.Node node = script.GetNearest (ray);
	...
	public void DrawUserConnectionsWindow () {
		...
		if (selectedUserConnection >= 0 && selectedUserConnection < conns.Length && Event.current.type != EventType.Used) {
			...
			if(GUILayout.Button("Snap to closest nodes")) {
				Pathfinding.Node node1 = script.GetNearest (conn.p1).node;
				Pathfinding.Node node2 = script.GetNearest (conn.p2).node;
